...
%==========================================================================
    ...
    % user choose path
    prompt = (['Which path do you want to follow ?',...
             '\nEnter 1 for the circle, 2 for the first path, 3 for the second one and 4 for the last.',...
             '\nTo exit press 0\n']);
    user_path = input(prompt);
    if user_path == 0
        error('Successful exit');
    else
        chosen_path = path_options{user_path};
        load(chosen_path);
